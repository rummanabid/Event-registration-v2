...
router.post('/events', (req, res) => {
  ...
  db.prepare('INSERT INTO events (id, name, slug, date, location, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(id, name, slug, date || null, location || null, now);
...
router.put('/events/:id', (req, res) => {
  ...
  db.prepare('UPDATE events SET name = ?, date = ?, location = ? WHERE id = ?').run(name, date || null, location || null, req.params.id);
...
router.post('/register', async (req, res) => {
  ...
  db.prepare(`
    INSERT INTO registrations (id, event_id, full_name, email, phone, company, custom_data, qr_token, checked_in, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)
  `).run(id, event_id, full_name, email, phone || null, company || null, custom_data ? JSON.stringify(custom_data) : null, qrToken, now);

  const baseUrl = getBaseUrl(req);
  const qrUrl = `${baseUrl}/checkin/${qrToken}`;
  const qrDataUrl = await generateQRDataURL(qrUrl);

  res.status(201).json({ id, qrToken, qrDataUrl, qrUrl });
});
...
router.post('/events/:id/field-config', (req, res) => {
  ...
  const upsert = db.prepare(`
    INSERT INTO field_config (id, event_id, field_name, is_visible, is_required)
    VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(event_id, field_name) DO UPDATE SET is_visible = excluded.is_visible, is_required = excluded.is_required
  `);

  // Check if unique constraint exists, if not recreate with it
...
router.post('/events/:id/form-fields', (req, res) => {
  ...
  const save = db.transaction(() => {
    ...
    fields.forEach((f, i) => {
      db.prepare(`
        INSERT INTO form_fields (id, event_id, field_type, label, is_required, options, sort_order)
        VALUES (?, ?, ?, ?, ?, ?, ?)
...
router.get('/qr/:qrToken.png', async (req, res) => {
  ...
  const reg = db.prepare('SELECT * FROM registrations WHERE qr_token = ?').get(token);
...
router.get('/events/:id/checkin-count', (req, res) => {
  const db = getDb();
  const row = db.prepare('SELECT COUNT(*) as total, SUM(checked_in) as checked_in FROM registrations WHERE event_id = ?').get(req.params.id);
