...
router.post('/events', (req, res) => {
  const db = getDb();
  const { name, date, location, maps_url, capacity } = req.body;
  ...
  db.prepare('INSERT INTO events (id, name, slug, date, location, maps_url, capacity, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, name, slug, date || null, location || null, maps_url || null, capacity ? parseInt(capacity) : null, now);
...
router.put('/events/:id', (req, res) => {
  ...
  db.prepare('UPDATE events SET name = ?, date = ?, location = ?, maps_url = ?, capacity = ? WHERE id = ?').run(name, date || null, location || null, maps_url || null, capacity ? parseInt(capacity) : null, req.params.id);
